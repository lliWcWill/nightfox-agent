...
LOG_FILE="${ROOT_DIR}/nightfox.${MODE}.log"

DEV_PATTERNS=(
  "tsx watch src/index.ts"
  "node .*nightfox/node_modules/.bin/tsx"
  "tsx/dist/loader"
  "npm run dev"
...
function status() {
  if pids=$(list_pids 2>/dev/null); then
    echo "Nightfox (${MODE}) is running:"
    echo "${pids}" | sed 's/^/  PID: /'
    return 0
  fi

  echo "Nightfox (${MODE}) is not running."
  return 1
}
...
function stop() {
  if ! pids=$(list_pids 2>/dev/null); then
    echo "No Nightfox (${MODE}) process found."
    return 0
  fi

  echo "Stopping Nightfox (${MODE})..."
  ...
  if ! wait_for_stop 10; then
    echo "Warning: Nightfox (${MODE}) did not fully stop within timeout."
  fi
}

function stop_all() {
  if ! pids=$(list_pids_all 2>/dev/null); then
    echo "No Nightfox processes found."
    return 0
  fi

  echo "Stopping all Nightfox processes..."
  echo "${pids}" | xargs -r kill -TERM
  sleep 1
  ...
  if ! wait_for_stop_all 10; then
    echo "Warning: Nightfox processes did not fully stop within timeout."
  fi
...
function start() {
  if status >/dev/null 2>&1; then
      echo "Nightfox (${MODE}) already running."
    status
    return 0
  fi

  echo "Starting Nightfox (${MODE})..."
  cd "${ROOT_DIR}"
  ...
  if ! wait_for_start 10; then
    echo "Warning: Nightfox (${MODE}) did not appear to start."
  fi
  status || true
  echo "Log: ${LOG_FILE}"
}
